...
var handlers = {
  ...
  processString: function(){
    ...
    var wordsToTransalte = textToTranslate.value.split(" ");
    for (var i = 0; i < words.word.length; i++){
        for (var j = 0; j < wordsToTransalte.length; j++){
          if (words.word[i].sourceWord === wordsToTransalte[j]){
            knownWords.push(words.word[i].translationWord);
          }
        }
    }
    translationResults.innerHTML = knownWords.join(" ");
   }
}
...
var wordsUl = document.getElementById('known-translations');
